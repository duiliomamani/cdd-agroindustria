...
cultives_dict = {c['name']: c for c in cultives}

data_agro = common.read_csv(f'{INPUT_DIR}/data_agro_v1.csv')

meteo_dict = {}
for file in os.listdir(OUTPUT_DIR + "/weather"):
    meteo_dict[file] = common.read_json(OUTPUT_DIR + "/weather/" + file)

for row in data_agro:
        print(f'Processing {row['index']} of {len(data_agro)}')
...
common.write_csv(f'{OUTPUT_DIR}/data_agro.csv', data_agro)